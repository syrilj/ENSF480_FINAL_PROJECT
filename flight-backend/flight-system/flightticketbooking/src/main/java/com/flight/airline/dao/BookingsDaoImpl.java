...
public class BookingsDaoImpl implements BookingsDao {
	...
	public double addPassenger(String p_fno, String p_from, String p_to, Date p_dedate, Date p_ardate, String p_detime,
		...
		String Query = "insert into passenger_details(p_pnr,p_name,p_seatno,p_sex,p_fno,p_from,p_to,p_dedate,p_ardate,p_detime,p_artime,p_status,p_class,p_email,p_bookingdate) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			stmt.setString(14, p_email);
			stmt.setString(15, todaysDate);
			stmt.execute();
		} catch (SQLException e) {
			e.printStackTrace();
		}
